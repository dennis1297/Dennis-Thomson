          ...
          <div class="panel">
			...
			<script>
				...
				function delopval(id)
				{
					...
					if(n)
						$.get("<?php echo ADMINURL?>actions/script.php?action=deleopval&table=tbl_options_value&columnname=op_val_id&id="+id, resopval);				
				}
			...
			if(isset($_REQUEST['addopt']))
			{
				...
				if(isset($_GET["opid"])){
					...
					$ins=mysqli_query($conn,"update tbl_options set op_name='$optname' where op_id=".$_GET["opid"]."");
					...
					if($_POST['optval_id']!=''){
						...
						{							
							...
							if($optval_name!='')
							{
								...
								if($optval_id=='')
									$ins = mysqli_query($conn,"insert into tbl_options_value (op_id,op_val_name) values('".$_GET["opid"]."','$optval_name')");   
							}
						}						
					} 
					foreach ($_POST['optval_name'] as $key => $value) 
					{
						...
						if($optval_name!='')
							$ins = mysqli_query($conn,"update tbl_options_value set op_id='".$_GET["opid"]."',op_val_name='$optval_name' where op_val_id='$optval_id'");   
					}
				}
				else{
					
					$ins=mysqli_query($conn,"insert into tbl_options (op_name) value('$optname')");
					...
					foreach ($_POST['optval_name'] as $key => $value) 
					{
						...
						if($optval_name!='')
							$ins = mysqli_query($conn,"insert into tbl_options_value(op_id,op_val_name) values('$opt_id','$optval_name')");   
					}
					
				}
							
				if($ins)
					echo '<script>alert("Updated successfully"); window.location.href="'.ADMINURL.'view-options";</script>';
				else 
					echo '<script>alert("Error..");window.location.href="'.ADMINURL.'view-options";</script>';
			}
			?>
